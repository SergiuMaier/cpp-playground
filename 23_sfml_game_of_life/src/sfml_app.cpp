...
void SfmlApp::updateMatrixCellAndUI(unsigned cell_x, unsigned cell_y)
{
	// by incrementing the coordinates with 1 I avoid messing the extra layer of dead cells in the matrix 

	if (cell_x < (matrix[1].size() - 2) && cell_y < (matrix.size() - 2)) //by pressing the black lines on the UI the user can mess the extra border of cells
																		 //I don't want that
	{
...
void SfmlApp::update_UI_with_next_generation()
{
	...
	for (size_t index_lines = 1; index_lines < (matrix.size() ); ++index_lines) 
	{
		...
		y_ui++;
		x_ui = 0; //go to the next line and reset the column coordinate to 0
	}
}
...
//default values in case something goes wrong when I try to get the info from the file
